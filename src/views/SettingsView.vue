...
import type { Goal, BodyMetric, ProteinIntake, PersonalBest } from '@/lib/database.types'
...
async function exportData() {
  ...
  try {
    // Fetch all user data
    const [goalsRes, metricsRes, proteinRes, pbsRes] = await Promise.all([
      supabase.from('goals').select('*').eq('user_id', auth.user.id),
    ...
    // Create CSV content
    let csv = '=== TRACKD DATA EXPORT ===\n'
    csv += `Exported: ${new Date().toISOString()}\n\n`

    // Goals
    csv += '=== GOALS ===\n'
    csv += 'Description,Reward,Target Date,Created,Completed\n'
    for (const goal of ((goalsRes.data || []) as Goal[])) {
      csv += `"${goal.description}","${goal.reward || ''}",${goal.target_date},${goal.created_at},${goal.completed_at || ''}\n`
    }
    ...
    a.href = url
    a.download = `trackd-export-${new Date().toISOString().split('T')[0]}.csv`
...
<template>
  <div class="min-h-screen bg-slate-900 text-white">
    ...
    <main class="p-4">
      <!-- Appearance -->
      <section class="mb-6">
        <h2 class="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-3">Appearance</h2>
          ...
          >
            <div class="flex items-center gap-3">
              <svg class="w-5 h-5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path v-if="settings.theme === 'dark'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
          ...
          <div class="px-4 py-3">
            ...
            <div class="flex gap-2">
              <button
                ...
                :class="settings.weightUnit === 'imperial'
              ...
              <button
                ...
                class="flex-1 py-2 rounded-lg font-medium transition-colors"
                :class="settings.weightUnit === 'metric'
                  ? 'bg-emerald-600 text-white'
                  : 'bg-slate-700 text-slate-400 hover:bg-slate-600'"
              >
                Metric (kg)
